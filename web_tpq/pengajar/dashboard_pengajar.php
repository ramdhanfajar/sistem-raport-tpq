...
if ($result_profil->num_rows > 0) {
    ...
    $nama_pengajar = $data_profil['nama_lengkap'];
    $file_foto_pengajar = $data_profil['foto'];
    $_SESSION['nama_lengkap'] = $nama_pengajar;
}
$stmt_profil->close();

// --- 3. AMBIL JADWAL MENGAJAR (DARI TABEL jadwal_pelajaran) ---
// Ini akan otomatis menampilkan apa yang sudah di-plot oleh Admin
...
if ($id_pengajar_db) {
    $sql_jadwal = "SELECT 
                   ...
                   ORDER BY jp.tahun_ajaran DESC, k.nama_kelas ASC, m.nama_materi ASC";

    $stmt_jadwal = $koneksi->prepare($sql_jadwal);
    $stmt_jadwal->bind_param("i", $id_pengajar_db);
    $stmt_jadwal->execute();
    $result_jadwal = $stmt_jadwal->get_result();

    while ($row = $result_jadwal->fetch_assoc()) {
        $data_jadwal_mengajar[] = $row;
    }
    $stmt_jadwal->close();
}
...
<head>
    ...
    <style>
        /* === CSS SAMA SEPERTI FILE LAIN (LAYOUT HP/RESPONSIF) === */
        :root {
            --warna-hijau: #00a86b;
        ...
        *, *:before, *:after { box-sizing: inherit; }
        ...
        .overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); z-index: 999; opacity: 0; visibility: hidden; transition: opacity 0.3s ease-out; }
        ...
        .user-profile .icon-wrapper { background-color: white; color: var(--warna-hijau); border-radius: 50%; width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; margin-left: 10px; overflow: hidden; }
        ...
        @media (min-width: 992px) {
            ...
            .card-welcome, .table-card { grid-column: 1 / -1; }
        }
        
        .card { background-color: white; padding: 20px; border-radius: 15px; box-shadow: 0 6px 15px rgba(0, 0, 0, 0.07); }
        .card-welcome { background-color: var(--warna-hijau-muda); color: var(--warna-hijau); font-weight: 500; }
        .card-welcome h2 { margin: 0 0 5px 0; font-size: 1.2rem; }
        .card-welcome p { margin: 0; font-size: 0.9rem; line-height: 1.5; }

        .table-card { padding: 20px; background-color: white; border: 1px solid #ddd; overflow-x: auto; }
        .table-card h3 { color: #d90000; font-size: 1.1rem; font-weight: 700; margin-top: 0; margin-bottom: 15px; }
        .table-card table { width: 100%; border-collapse: collapse; font-size: 0.9rem; border: 2px solid #777; }
        .table-card th, .table-card td { padding: 10px 15px; text-align: left; border: 1px solid #aaa; white-space: nowrap; }
        .table-card th { background-color: #EFEFEF; color: #333; font-weight: 600; text-align: center; }
        .table-card td { color: var(--warna-teks-abu); }
        .table-card .no-data { text-align: center; padding: 20px; font-style: italic; }
        
        @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .card-welcome, .table-card { animation: fadeIn 0.5s ease-out; }
        
        @media (max-width: 480px) { .user-profile .user-info { display: none; } .user-profile .icon-wrapper { margin-left: 0; } }
    </style>
</head>
<body>
            ...
            <div class="dashboard-grid">
                ...
                <div class="card table-card">
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php if (!empty($data_jadwal_mengajar)): ?>
                                <?php $no = 1; foreach ($data_jadwal_mengajar as $jadwal): ?>
                                <tr>
                                    <td style="text-align: center;"><?php echo $no++; ?>.</td>
                                    <td><?php echo htmlspecialchars($jadwal['nama_materi']); ?></td>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="no-data">
                                        Belum ada jadwal mengajar yang ditetapkan Admin.
                                    </td>
                                </tr>
                            <?php endif; ?>
                            
                            <?php 
                            $baris_tersisa = 4 - count($data_jadwal_mengajar);
                            if ($baris_tersisa < 0) $baris_tersisa = 0;
                            for ($i = 0; $i < $baris_tersisa; $i++): 
                            ?>
                                <tr>
                                    <td style="text-align: center;"><?php if (!empty($data_jadwal_mengajar)) echo ($no + $i) . '.'; else echo '&nbsp;'; ?></td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                </tr>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!sessionStorage.getItem('dashboardWelcomed')) {
                Swal.fire({ title: 'Selamat Datang!', text: `Halo, ${namaPengajar}. Selamat bertugas.`, icon: 'success', timer: 2500, showConfirmButton: false, timerProgressBar: true });
